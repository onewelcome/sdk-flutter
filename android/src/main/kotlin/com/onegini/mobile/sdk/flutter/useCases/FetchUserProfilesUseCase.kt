...
class FetchUserProfilesUseCase(private val oneginiClient: OneginiClient) {
    operator fun invoke(result: MethodChannel.Result) {
        val userProfiles = oneginiClient.userClient.userProfiles
        val userProfileArray = fetchUserProfileArray(userProfiles)
        result.success(Gson().toJson(userProfileArray))
    }

    private fun fetchUserProfileArray(userProfiles: Set<UserProfile?>?): ArrayList<Map<String, Any>> {
