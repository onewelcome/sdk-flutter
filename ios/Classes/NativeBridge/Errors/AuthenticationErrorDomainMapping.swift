...
class AuthenticationErrorDomainMapping {
    ...
    func mapErrorWithCustomInfo(_ customInfo: ONGCustomInfo) -> SdkError {
        if customInfo.status >= 4000 && customInfo.status < 5000 {
            ...
            return SdkError(title: title, errorDescription: message, code: customInfo.status)
        } else {
            return SdkError(errorDescription: "Something went wrong.", code: customInfo.status)
        }
    }
    ...
    func mapError(_ error: Error) -> SdkError {
        ...
        case ONGAuthenticationError.authenticatorDeregistered.rawValue:
            ...
            return SdkError(title: title, errorDescription: message, recoverySuggestion: recoverySuggestion, code: error.code)
        ...
        case ONGAuthenticationError.touchIDAuthenticatorFailure.rawValue:
            ...
            return SdkError(title: title, errorDescription: message, code: error.code)
