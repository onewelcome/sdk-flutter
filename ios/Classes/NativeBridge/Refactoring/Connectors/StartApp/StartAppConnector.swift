...
class StartAppConnector: StartAppConnectorProtocol {
    ...
    init(wrapper: StartAppWrapperProtocol = StartAppWrapper()) {
        startAppWrapper = wrapper
    }
    ...
    func startApp(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        startAppWrapper.startApp(timeInterval: connectionTimeout, callback: { [weak self] success, error in
