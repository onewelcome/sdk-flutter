...
class ResourcesConnector {
    ...
    init(wrapper: ResourcesWrapperProtocol = ResourcesWrapper()) {
    ...
    private func generateONGResourceRequest(from parameters: [String: Any]) -> ONGResourceRequest {
        let encoding = getEncodingByValue((parameters[Constants.Parameters.encoding] as? String) ?? Constants.Encoding.url)
        let path = (parameters[Constants.Parameters.path] as? String) ?? ""
        let method = (parameters[Constants.Parameters.method] as? String) ?? "GET"
        ...
        if let body = parameters[Constants.Parameters.body] as? String {
            ...
            request = ONGResourceRequest(path: path, method: method, body: data, headers: headers)
        } else {
            request = ONGResourceRequest(path: path, method: method, parameters: parameters[Constants.Parameters.parameters] as? [String: Any], encoding: encoding, headers: headers)
        }
    ...
    private func implicitResourceRequest(_ request: ONGResourceRequest, parameters: [String: Any], callback: @escaping ResourcesCallbackResult, result: @escaping FlutterResult) {
        guard let userProfile = resourcesWrapper.authenticatedUserProfile() else {
            result(FlutterError.from(customType: .noUserAuthenticated))
            return
        }

        let scopes = parameters[Constants.Parameters.scope] as? [String] ?? [String]()
        resourcesWrapper.authenticateImplicitly(profile: userProfile, scopes: scopes) { [weak self] _, error in
    ...
    private func authenticateDeviceForResource(scopes: [String], result: @escaping FlutterResult) {
        resourcesWrapper.authenticateDevice(scopes: scopes) { success, error in
            guard let error = error else {
                result(success)
                return
            }
...
extension ResourcesConnector: ResourcesConnectorProtocol {
    func fetchResources(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        let completionCallback: ResourcesCallbackResult = { response, error in
            ...
            // 2.
            guard let data = response?.data, let responseData = String(data: data, encoding: .utf8) else {
        ...
        case Constants.Routes.getAnonymousResource:
            resourcesWrapper.anonymousRequest(request, parameters: parameters, callback: completionCallback)
        case Constants.Routes.getResource:
            resourcesWrapper.authenticatedRequest(request, parameters: parameters, callback: completionCallback)
        case Constants.Routes.getUnauthenticatedResource:
            resourcesWrapper.unauthenticatedRequest(request, parameters: parameters, callback: completionCallback)
        case Constants.Routes.getImplicitResource:
            implicitResourceRequest(request, parameters: parameters, callback: completionCallback, result: result)
        case Constants.Routes.authenticateDeviceForResource: do {
                let scopes = parameters[Constants.Parameters.scope] as? [String] ?? [String]()
                authenticateDeviceForResource(scopes: scopes, result: result)
            }
        default:
            result(FlutterError.from(customType: .invalidArguments))
        }
