...
class UserClientApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: UserClientApi?) {
    ...
    if let api = api {
      isBiometricAuthenticatorRegisteredChannel.setMessageHandler { _, reply in
        api.isBiometricAuthenticatorRegistered() { result in
          switch result {
            case .success(let res):
              reply(wrapResult(res))
            case .failure(let error):
    ...
    if let api = api {
      getPreferredAuthenticatorChannel.setMessageHandler { _, reply in
        api.getPreferredAuthenticator() { result in
          switch result {
            case .success(let res):
              reply(wrapResult(res))
            case .failure(let error):
