 ...
 *  This method is using preferred authenticator to authenticate.
 *
 ...
 */
- (void)authenticateUser:(ONGUserProfile *)userProfile delegate:(id<ONGAuthenticationDelegate>)delegate DEPRECATED_MSG_ATTRIBUTE("Use authenticateUser:authenticator:delegate: method instead.");

/**
 *  One of two entry points into the authentication process.
 *  
 *  @param authenticator authenticatior used to authenticate
 *  @param profile profile to authenticate
 *  @param delegate authentication delegate, ONGUserClient keeps weak reference on delegate to avoid retain cycles
 */
- (void)authenticateUserWithAuthenticator:(ONGAuthenticator *)authenticator profile:(ONGUserProfile *)profile delegate:(id<ONGAuthenticationDelegate>)delegate DEPRECATED_MSG_ATTRIBUTE("Use authenticateUser:authenticator:delegate: method instead.");
 ...
 */
- (BOOL)canHandlePushMobileAuthRequest:(NSDictionary *)userInfo DEPRECATED_MSG_ATTRIBUTE("Use pendingMobileAuthRequestFromUserInfo: method instead.");
...
/**
 *  When a push notification is received by the application, the notification must be forwarded to the client.
 *  The client will then fetch the actual encrypted payload and invoke the delegate with the embedded message.
 *
 *  This should be invoked from the UIApplicationDelegate
 ...
 *  @param delegate delegate responsible for handling push messages
 */
- (void)handlePushMobileAuthRequest:(NSDictionary *)userInfo delegate:(id<ONGMobileAuthRequestDelegate>)delegate DEPRECATED_MSG_ATTRIBUTE("Use handlePendingPushMobileAuthRequest:delegate: method instead.");
